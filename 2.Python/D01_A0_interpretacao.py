###############################################################################
# ADINs - base original sem covariáveis [A] - dim 1
descarte_votantes = False
###############################################################################
import sys
import os
import pandas as pd
...
f_cria_DE_PARA(dados=DD,
               ...
               path=os.path.join(diretorio,'3.Auxiliares','D01. AB DE_PARA.csv'))
...
modelo = f_constroi_arquitetura(
    ...
    regpar_eff=10**(-3),regpar_latU=10**(-7),regpar_latP=10**(-7),
...
modelo = f_treina_modelo(
    ...
    paciencia=6,ini_epoch=0,add_epoch=100,validation_split=0.1,
...
intercepto = modelo.get_weights()[2][0][0]
...
for pos in range(dados_valid.shape[0]):
    i = int(dados_valid.iloc[pos,2])
    j = int(dados_valid.iloc[pos,0])
    eta_ij = intercepto+alpha_votante[i]+alpha_votacao[j]
...
# LE DE-PARA
caminho = os.path.join(diretorio,'3.Auxiliares','D01. AB DE_PARA.csv')
...
depara_aux['Descricao'] = [x[0:-1] for x in depara_aux['Descricao']]
depara_aux['alpha'] = alpha_votante
depara_aux.columns = ['id','votante','alpha']
# Salva para ser processado no R
caminho_out = os.path.join(diretorio,'3.Auxiliares','D01_A0_Votante.csv')
...
depara_aux = depara \
    ...
    .drop(['Variavel'],axis=1) \
    .sort_values(['ID'],ascending=True)
depara_aux['Descricao'] = [x[7:] for x in depara_aux['Descricao']]
depara_aux['alpha'] = alpha_votacao
depara_aux.columns = ['id','votacao','alpha']
# Salva para ser processado no R
caminho_out = os.path.join(diretorio,'3.Auxiliares','D01_A0_Votacao.csv')
depara_aux.to_csv(caminho_out,index=False)
