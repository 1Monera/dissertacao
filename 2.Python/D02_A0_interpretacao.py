...
for pos in range(dados_valid.shape[0]):
    i = int(dados_valid.iloc[pos,5])
    j = int(dados_valid.iloc[pos,7])
...
# LE DE-PARA
caminho = os.path.join(diretorio,'3.Auxiliares','D02. AB DE_PARA.csv')
...
caminho_out = os.path.join(diretorio,'3.Auxiliares','D02_A0_Votante.csv')
...
# Salva para ser processado no R
caminho_out = os.path.join(diretorio,'3.Auxiliares','D02_A0_Votacao.csv')
depara_aux.to_csv(caminho_out,index=False)
