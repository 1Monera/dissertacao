...
f_cria_DE_PARA(dados=DD,
               ...
               path=os.path.join(diretorio,'3.Auxiliares','D02. CD DE_PARA.csv'))
...
var_eff_num = []
var_eff_cat = ['Votante','Presidente','Magistratura','Politica','Votacao','UF','Relator','TEMA','AMBITO','RECREQ']
 # Votante
var_latU_num = []
var_latU_cat = ['Votante','Presidente','Magistratura','Politica']
# Votacao
var_latP_num = []
var_latP_cat = ['Votacao','UF','Relator','TEMA','AMBITO','RECREQ']
...
optimizer.max
'''
{'target': -0.5282333024503434,
 'params': {'dim_latente': 2.0,
  'exp_p': 1.0,
  'expoente_batch_size': 7.1229914522553965,
  'learning_rate': -1.4751773128994476,
  'paciencia': 7.831246984834069,
  'regpar_eff': -5.23340652444012,
  'regpar_latP': -6.458799326473565,
  'regpar_latU': -3.0,
...
modelo = f_constroi_arquitetura(
    ...
    regpar_eff=10**(-5.23340652444012),regpar_latU=10**(-3),regpar_latP=10**(-6.458799326473565),
    variaveis=variaveis,learning_rate=10**(-1.4751773128994476))
modelo = f_treina_modelo(
    modelo=modelo,dados=dados_train,exp_p=1,variaveis=variaveis,
    paciencia=1,ini_epoch=0,add_epoch=100,validation_split=0.1,
