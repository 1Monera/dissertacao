###############################################################################
# sen90 - base sem votações x com covariáveis [D] - dim 2
###############################################################################
import sys
import os
import pandas as pd
...
# Leitura da base
caminho = os.path.join(diretorio,'0.Dados','D00. Matriz sem votações NA.csv')
...
caminho = os.path.join(diretorio,'5.Gráficos','D00.CD_Proporcao_Votos_Originais.png')
...
f_cria_DE_PARA(dados=CC,
               ...
               path=os.path.join(diretorio,'3.Auxiliares','D00. CD DE_PARA.csv'))
...
var_eff_num = []
var_eff_cat = ['Votante','Partido','Votacao']
 # Votante
var_latU_num = []
var_latU_cat = ['Votante','Partido']
# Votacao
var_latP_num = []
...
optimizer.max
'''
{'target': -0.4712131301274952,
 'params': {'dim_latente': 2.0,
  'exp_p': 1.0,
  'expoente_batch_size': 6.23624459267946,
  'learning_rate': -1.3010299956639813,
  'paciencia': 8.584060855627461,
  'regpar_eff': -4.296782492764297,
  'regpar_latP': -7.0,
  'regpar_latU': -7.0,
...
modelo = f_constroi_arquitetura(
    ...
    regpar_eff=10**(-4.296782492764297),regpar_latU=10**(-7),regpar_latP=10**(-7),
