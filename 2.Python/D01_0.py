###############################################################################
# ADINs - base original com covariáveis [B] - dim 1
...
os.chdir(r'C:/Users/Maurício/Google Drive/Estudo/Mestrado/Pesquisas/Dissertação/Códigos V3/2.Python')
execfile('Funcoes_DE_PARA.py')
f_cria_DE_PARA(dados=DD,
               nomes_colunas=['Votante','Votacao','Autora_entidade'],
               path=os.path.join(diretorio,'3.Auxiliares','D01. DE_PARA.csv'))
...
var_eff_cat = ['Votante','Votacao','Autora_entidade']
 # Votante
var_latU_num = []
var_latU_cat = ['Votante']
# Votacao
var_latP_num = []
var_latP_cat = ['Votacao','Autora_entidade']
...
optimizer.max
'''
{'target': -0.5507266288213916,
 'params': {'dim_latente': 1.0,
  'exp_p': 1.0,
  'expoente_batch_size': 4.480413845197214,
  'learning_rate': -2.627697653604277,
  'paciencia': 13.884427421298016,
  'regpar_eff': -6.767296134052593,
  'regpar_latP': -5.475132288726778,
  'regpar_latU': -5.076222926556428,
  'validation_split': 0.0749146293893275}}
'''

# Aplicando modelo mais adequado
modelo = f_constroi_arquitetura(
    dados=dados_full,dim_latente=1,exp_p=1,reg_type='L1',
    regpar_eff=10**(-6.767296134052593),regpar_latU=10**(-5.076222926556428),regpar_latP=10**(-5.475132288726778),
    variaveis=variaveis,learning_rate=10**(-2.627697653604277))
modelo = f_treina_modelo(
    modelo=modelo,dados=dados_train,exp_p=1,variaveis=variaveis,
    paciencia=14,ini_epoch=0,add_epoch=500,validation_split=0.0749146293893275,
