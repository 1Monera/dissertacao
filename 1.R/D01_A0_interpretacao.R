...
# Votante
caminho = file.path(diret,'3.Auxiliares','D01_A0_Votante.csv')
df = read.csv(caminho,sep=',')
#df[abs(df$alpha) <= 0.05,"votante"] = ""
df$Jitter = 0

setwd(diret)
pdf("8.Extra/D01_Graficos_FM-COV_Votante.pdf",width=8,height=4)
df %>%
...
# Votacao
caminho = file.path(diret,'3.Auxiliares','D01_A0_Votacao.csv')
df = read.csv(caminho,sep=',')
#df[!(df$id %in% c(168)),"votacao"] = ""
df$Jitter = 0

setwd(diret)
pdf("8.Extra/D01_Graficos_FM-COV_Votacao.pdf",width=8,height=4)
df %>%
